...
**Descripción:**: Proyecto Spring Boot que implementa procesos batch y una API REST mínima. Usa Spring Batch, Spring Data JPA y MySQL.
...
- `docker-compose.yml`: levanta `db` (MySQL) y `app`. Copia `.env.example` a `.env` antes de levantar.
